function [info,msg] = imtifinfo_immc(filename)
...
%   $Revision: 1.1.6.4 $  $Date: 2006/10/04 22:54:33 $
%   This version revised By Sjoerd Ligthart to handle scaling in
%   IMMC/Veridex Tiff scaling
...
info.TileByteCounts = [];
% new!
info.LowerStretchLimit = [];
info.HigherStretchLimit = [];
...
while (nextIFDOffset ~= 0)
    ...
    info(k).LowerStretchLimit = 0;
    info(k).HigherStretchLimit = 1;
    ...
    for p = 1:tagCount
        ...
        case 755
            % LowerStretchLimit
            fseek(fid, 6, 'cof');
            info(k).LowerStretchLimit = fread(fid, 1, 'uint16');    
            
        case 756
            % HigherStretchLimit
            fseek(fid, 6, 'cof');
            info(k).HigherStretchLimit = fread(fid, 1, 'uint16');    
